...
def connect_until_one_circuit(boxes):
    pairs = get_pairwise_distances(boxes)
...
class MyTest(unittest.TestCase):
    ...
    def test_connect_until_one_circuit(self):
        boxes = parse_boxes("test_input.txt")

        final_pair = connect_until_one_circuit(boxes)

        self.assertEqual(216, final_pair[0].x)
        self.assertEqual(117, final_pair[1].x)
        self.assertEqual(25272, final_pair[0].x * final_pair[1].x)
...
final_pair = connect_until_one_circuit(boxes)
answer = final_pair[0].x * final_pair[1].x
