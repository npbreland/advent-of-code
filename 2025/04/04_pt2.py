...
class Grid:
    ...
    def get_accessible_rolls_with_removal(self):
        total = 0

        while(True):
            subtotal, rolls = self.get_accessible_rolls()
            ...
            for roll in rolls:
                self.remove_roll(roll[0], roll[1])

        return total

    def remove_roll(self, x, y):
        self.grid[y][x] = "."
...
class MyTest(unittest.TestCase):
    ...
    def test_get_accessible_rolls(self):
        grid = Grid("test_input.txt")
        total, rolls = grid.get_accessible_rolls()
        self.assertEqual(13, total)
        self.assertEqual((2, 0), rolls[0])
        self.assertEqual((3, 0), rolls[1])
        self.assertEqual((5, 0), rolls[2])

    def test_remove_roll(self):
        grid = Grid("test_input.txt")
        self.assertEqual("@", grid.grid[0][2])
        grid.remove_roll(2, 0)
        self.assertEqual(".", grid.grid[0][2])

    def test_get_accessible_rolls_with_removal(self):        
        grid = Grid("test_input.txt")
        total = grid.get_accessible_rolls_with_removal()
...
# unittest.main()
grid = Grid("input.txt")
print("Accessible rolls:", grid.get_accessible_rolls_with_removal())
